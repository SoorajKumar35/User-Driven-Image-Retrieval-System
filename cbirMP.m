...
needed_feature_vectors = []

for i = 1:size(handles.posInds)
    needed_feature_vectors(:,i) = handles.META_DATA(:,handles.posInds(i));
end

% averaged_nfv = needed_feature_vectors/sum(needed_feature_vectors(:));
% [num_rows_anfv, num_cols_anfv] = size(averaged_nfv);
% [p,q] = ndgrid(1:num_rows_anfv, 1:num_cols_anfv);
%    centroid=[dot(p(:),averaged_nfv(:)),  dot(q(:),averaged_nfv(:))]  

for i = 1:47
    centroid(i,1) = (sum(needed_feature_vectors(i,:))/size(needed_feature_vectors(i,:),2));
end
...
D = []
for i = 1:1400
    image_feature_vector = handles.META_DATA(:,i)
    D(i) = ((image_feature_vector-centroid).')*W*(image_feature_vector-centroid);
end
%return rank-sorted list of top indices (#3)
sorted_D = sort(D);
for i=1:20
    handles.currentTopInds(i) = find(sorted_D(i) == D)
end
...
%good luck!
if(size(handles.posInds,2) == 1)
    W = eye(47)
    return;
else
    for i=1:47
        for k=1:size(handles.posInds,2)
            %Get the K values
            K_vals(k,1) = handles.META_DATA(i,k); 
            K_vals_variance = var(K_vals);
            %Regularizing
            K_vals_variance = K_vals_variance + 0.0222;
            W(i,i) = 1/(K_vals_variance);
        end
    end
    return;
end
